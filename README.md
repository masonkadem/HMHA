...
5. There is no optimal delay. tau over 0, 1, 5, 20 spans 0.00010 in converged loss with
   overlapping CIs; only tau = 100 is worse. The predicted failure mode at long tau is
   absent: perfused-set changes per step over the held phase are 0.00 at every tau, so
   there is no gate oscillation for a longer lag to trade against.
