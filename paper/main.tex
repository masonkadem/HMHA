...
\begin{figure*}[t]
  ...
  true circuit size, median over the converged phase. Both autoregulated arms give
  identical $B$ and are drawn as one line, which lies on the identity; the stall gate's
  contribution is role recovery, not count. The band spans the remaining open-loop rules.
  (b) Fitted $\mathrm{d}B/\mathrm{d}k^\star$ for every arm. Three open-loop rules have
  \emph{negative} slope. Poiseuille is the informative failure: its perfused set is
  identical to the fixed threshold's by construction and only the shares within it
  differ, yet it scores worse, so selection rather than allocation is the binding
  problem.}
  \label{fig:arms}
\end{figure*}
...
\begin{table}[t]
  ...
  \caption{Supply rules on \texttt{multi\_relation}, grouped by whether supply answers
  to task performance or only to the demand distribution. $H{=}32$, $d_k{=}32$, $N{=}16$,
